...
test_ehr_dataset = pickle.load(open('./data_image/testDataset.pkl', 'rb'))
test_ehr_dataset = [p for p in test_ehr_dataset if len(p['visits']) > 1]
test_ehr_dataset = [{'visits': [(v[0], v[1]) for v in p['visits']]} for p in test_ehr_dataset]
...
model.eval()
with torch.no_grad():
  for v_i in tqdm(range(0, len(test_ehr_dataset), config.plus_batch_size)):
    # Get batch inputs
    batch_ehr, batch_image, batch_mask = get_batch(v_i, config.plus_batch_size, 'test')
    ...
    # Get batch outputs
    test_loss, predictions, labels = model(batch_ehr, batch_image, position_ids=None, ehr_labels=batch_ehr, ehr_masks=batch_mask)
    ...
    rounded_preds = rounded_preds[:,:,config.cardiopulmonary_vocab_size:config.cardiopulmonary_vocab_size+config.pulmonary_vocab_size+config.pleural_vocab_size+config.miscellaneous_vocab_size]
    labels = labels[:,:,config.cardiopulmonary_vocab_size:config.cardiopulmonary_vocab_size+config.pulmonary_vocab_size+config.pleural_vocab_size+config.miscellaneous_vocab_size]
    predictions = predictions[:,:,config.cardiopulmonary_vocab_size:config.cardiopulmonary_vocab_size+config.pulmonary_vocab_size+config.pleural_vocab_size+config.miscellaneous_vocab_size]
    ...
    n_total_codes += (torch.sum(batch_mask,0).squeeze() * (config.pulmonary_vocab_size+config.pleural_vocab_size+config.miscellaneous_vocab_size)).cpu()
...
pickle.dump(intermediate, open("./results/modality_completion_stats/MediSim_Image_Plus_intermediate_results.pkl", "wb"))
...
pickle.dump(metrics_dict, open("./results/modality_completion_stats/MediSim_Image_Plus_Metrics.pkl", "wb"))
