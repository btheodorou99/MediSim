...
test_ehr_dataset = [{'visits': [(v[0], v[1][:config.n_embed_positions]) for v in p['visits']]} for p in test_ehr_dataset]
...
with torch.no_grad():
 for v_i in tqdm(range(0, len(test_ehr_dataset), config.batch_size)):
   # Get batch inputs
   batch_ehr, batch_note, batch_mask = get_batch(v_i, config.batch_size, 'test')
   ...
   # Get batch outputs
   test_loss, predictions, labels = model(batch_ehr, batch_note, position_ids=None, ehr_labels=batch_ehr, ehr_masks=batch_mask)
...
pickle.dump(intermediate, open("./results/temporal_completion_stats/MediSim_Note_Plus_intermediate_results_ss.pkl", "wb"))
...
pickle.dump(metrics_dict, open("./results/temporal_completion_stats/MediSim_Note_Plus_Metrics_ss.pkl", "wb"))
