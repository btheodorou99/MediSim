...
class GPTBase(nn.Module):
    def __init__(self, config):
        ...
        self.n_vocab = config.word_vocab_size

        self.word_embedding = nn.Embedding(config.word_vocab_size+1, config.n_embd)
        self.position_embedding = nn.Embedding(config.n_gen_positions, config.n_embd)
        block = Block(config.n_gen_positions, config, scale=True)
        self.h = nn.ModuleList([copy.deepcopy(block) for _ in range(config.n_layer)])
        self.ln_f = LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)

    def forward(self, contextEmbedding, input_words, position_ids=None, past=None):
        if past is None:
            past_length = 0
        ...
        if position_ids is None:
            position_ids = torch.arange(past_length, input_words.size(1) + past_length, dtype=torch.long,
                                        device=input_words.device)
            position_ids = position_ids.unsqueeze(0).expand(input_words.size(0), input_words.size(1))

        inputs_embeds = self.word_embedding(input_words)
        position_embeds = self.position_embedding(position_ids)
        hidden_states = inputs_embeds + position_embeds
        hidden_states = torch.cat((contextEmbedding, hidden_states), dim=1)
        presents = []
...
class DecoderModel(nn.Module):
    def __init__(self, config):
        ...
        self.pad_idx = config.word_vocab_size
        self.contextEmbedding = nn.Linear(config.code_vocab_size, config.n_embd)
        self.transformer = GPTBase(config)
        self.ehr_head = GPTHead(self.transformer.word_embedding.weight, config)
        self.set_tied()

    def set_tied(self):
        """Make sure we are sharing the embeddings"""
        self.ehr_head.set_embeddings_weights(self.transformer.word_embedding.weight)

    def forward(self, context, input_words, gen_loss=False, position_ids=None, past=None):
        contextEmbedding = self.contextEmbedding(context)
        contextLen = contextEmbedding.size(1)
        hidden_states, _ = self.transformer(contextEmbedding, input_words, position_ids, past)
        word_logits = self.ehr_head(hidden_states)[:,contextLen-1:]
        if gen_loss:    
            shift_logits = word_logits[..., :-1, :].contiguous()
            loss_fn = nn.CrossEntropyLoss(ignore_index=self.pad_idx)
            loss = loss_fn(shift_logits.view(-1, shift_logits.size(-1)), input_words.view(-1))
...
class NoteEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.n_layer = config.n_embed_layer
        self.n_embd = config.n_embd
        self.n_vocab = config.word_vocab_size

        self.word_embedding = nn.Embedding(config.word_vocab_size+1, config.n_embd)
        self.position_embedding = nn.Embedding(config.n_embed_positions, config.n_embd)
        block = Block(config.n_embed_positions, config, scale=True, autoregressive=False)
        self.h = nn.ModuleList([copy.deepcopy(block) for _ in range(config.n_embed_layer)])
        self.ln_f = LayerNorm(config.n_embd, eps=config.layer_norm_epsilon)
        
    def forward(self, input_words):
        input_words = input_words[:, 1:, :]
        bs, ts, ws = input_words.size()
        input_words = input_words.reshape(-1, ws)
        position_ids = torch.arange(0, input_words.size(1), dtype=torch.long, device=input_words.device)
        position_ids = position_ids.unsqueeze(0).expand(input_words.size(0), input_words.size(1))
        ...
        hidden_states = self.ln_f(hidden_states)
        hidden_states = hidden_states.view(bs, ts, ws, -1)
        cls_embed = hidden_states[:, :, 0]
        return torch.cat((torch.zeros(bs, 1, self.n_embd, device=cls_embed.device), cls_embed), dim=1)
...
class CoarseTransformerModel(nn.Module):
    ...
    def forward(self, input_visits, note_embds, position_ids=None, past=None):
        if past is None:
            past_length = 0
            past = [None] * len(self.h)
        else:
        ...
        combined_visits = torch.cat((input_visits, note_embds), dim=2)
...
class MediSimModel(nn.Module):
    def __init__(self, config):
        ...
        self.note_encoder = NoteEncoder(config)
    ...
    def forward(self, input_visits, input_notes, position_ids=None, ehr_labels=None, ehr_masks=None, past=None):
        note_embds = self.note_encoder(input_notes)
        hidden_states = self.transformer(input_visits, note_embds, position_ids, past)
    ...
    def sample(self, input_visits, input_notes, random=True, temp=1):
        sig = nn.Sigmoid()
        note_embds = self.note_encoder(input_notes)
        hidden_states = self.transformer(input_visits, note_embds)
        i = 0
    ...
    def addModalities(self, input_visits, input_notes, random=True):
        sig = nn.Sigmoid()
        note_embds = self.note_encoder(input_notes)
        hidden_states = self.transformer(input_visits, note_embds)
        i = self.diagnosis_vocab_size
        while i < self.diagnosis_vocab_size+self.procedure_vocab_size+self.medication_vocab_size:
            next_logits = self.ehr_head.sample(hidden_states, input_visits)
            next_probs = sig(next_logits)
            if random:
                visit = torch.bernoulli(next_probs)
            ...
            first_nonzero = nonzero.min()
            if i + first_nonzero < self.diagnosis_vocab_size+self.procedure_vocab_size+self.medication_vocab_size:
                input_visits[:,-1,i + first_nonzero] = visit[:,0,i + first_nonzero]
