...
with torch.no_grad():
  for v_i in tqdm(range(0, len(test_ehr_dataset), config.batch_size)):
    ...
    rounded_preds = rounded_preds[:,:,config.medication_vocab_size:config.medication_vocab_size+config.diagnosis_vocab_size+config.procedure_vocab_size]
    labels = labels[:,:,config.medication_vocab_size:config.medication_vocab_size+config.diagnosis_vocab_size+config.procedure_vocab_size]
    predictions = predictions[:,:,config.medication_vocab_size:config.medication_vocab_size+config.diagnosis_vocab_size+config.procedure_vocab_size]
    ...
    n_total_codes += (torch.sum(batch_mask,0).squeeze() * (config.diagnosis_vocab_size+config.procedure_vocab_size)).cpu()
...
pickle.dump(intermediate, open("./results/modality_completion_stats/MediSim_intermediate_results_ss.pkl", "wb"))
...
pickle.dump(metrics_dict, open("./results/modality_completion_stats/MediSim_Metrics_ss.pkl", "wb"))
