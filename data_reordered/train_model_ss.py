  ...
  for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
      ...
      if cur_val_loss < global_loss:
        ...
        torch.save(state, './save/medisim_model_ss')
        print('\n------------ Save best model ------------\n')
      else:
        curr_patience += 1

  model.load_state_dict(torch.load('./save/medisim_model_ss')['model'])
...
def get_quality_batch(dataset, loc, batch_size):
  ...
  for i, p in enumerate(ehr):
    ...
    for j, v in enumerate(visits):
      batch_ehr[i,j+1][v] = 1 # set the visit codes  
    batch_ehr[i,len(visits),config.code_vocab_size+1] = 1 # Set the final visit to have the end token
    ...
    for e in range(QUALITY_EPOCHS):
      ...
      with torch.no_grad():
        ...
        if cur_val_loss < quality_loss:
          ...
          torch.save(state, f'./save/quality_model')
          print('\n------------ Save best model ------------\n')

    model.load_state_dict(torch.load('./save/quality_model')['model'])
    return model
...
if os.path.exists("./save/medisim_model_ss"):
