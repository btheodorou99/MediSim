...
class MediSimModel(nn.Module):
    ...
    def sample(self, input_visits, random=True, temp=1):
        sig = nn.Sigmoid()
    ...
    def addModalities(self, input_visits, random=True):
        sig = nn.Sigmoid()
        hidden_states = self.transformer(input_visits)
        i = self.medication_vocab_size
        while i < self.medication_vocab_size+self.diagnosis_vocab_size+self.procedure_vocab_size:
            next_logits = self.ehr_head.sample(hidden_states, input_visits)
            next_probs = sig(next_logits)
            if random:
                visit = torch.bernoulli(next_probs)
            ...
            first_nonzero = nonzero.min()
            if i + first_nonzero < self.medication_vocab_size+self.diagnosis_vocab_size+self.procedure_vocab_size:
                input_visits[:,-1,i + first_nonzero] = visit[:,0,i + first_nonzero]
