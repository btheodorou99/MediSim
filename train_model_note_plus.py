...
from model_note import MediSimModel
...
train_ehr_dataset = [{'visits': [(v[0], v[1][:config.n_embed_positions]) for v in p['visits']]} for p in train_ehr_dataset]
val_ehr_dataset = pickle.load(open('data_note/valDataset.pkl', 'rb'))
val_ehr_dataset = [{'visits': [(v[0], v[1][:config.n_embed_positions]) for v in p['visits']]} for p in val_ehr_dataset]
...
def get_batch(loc, batch_size, mode):
  ...
  batch_note = np.ones((len(ehr), config.n_ctx, config.n_embed_positions)) * config.word_vocab_size
  ...
  return batch_ehr, batch_note, batch_mask
...
if os.path.exists("./save/medisim_model_note_plus"):
  print("Loading previous model")
  checkpoint = torch.load('./save/medisim_model_note_plus', map_location=torch.device(device))
...
for e in tqdm(range(config.epoch)):
  ...
  for i in range(0, len(train_ehr_dataset), config.batch_size):
    ...
    batch_ehr, batch_note, batch_mask = get_batch(i, config.batch_size, 'train')
    ...
    optimizer.zero_grad()
    loss, _, _ = model(batch_ehr, batch_note, position_ids=None, ehr_labels=batch_ehr, ehr_masks=batch_mask)
    loss.backward()
    optimizer.step()
    ...
    if i % (500*config.batch_size) == 0:
      ...
      with torch.no_grad():
        ...
        for v_i in range(0, len(val_ehr_dataset), config.batch_size):
          batch_ehr, batch_note, batch_mask = get_batch(v_i, config.batch_size, 'valid')
          batch_ehr = torch.tensor(batch_ehr, dtype=torch.float32).to(device)
          batch_note = torch.tensor(batch_note, dtype=torch.long).to(device)
          batch_mask = torch.tensor(batch_mask, dtype=torch.float32).to(device)
  
          val_loss, _, _ = model(batch_ehr, batch_note, position_ids=None, ehr_labels=batch_ehr, ehr_masks=batch_mask)
        ...
        if cur_val_loss < global_loss:
          ...
          torch.save(state, './save/medisim_model_note_plus')
          print('\n------------ Save best model ------------\n')