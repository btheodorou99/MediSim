...
if os.path.exists("./save/temporal/lstm"):
        ...
        if i % (500*config.batch_size) == 0:
            ...
            with torch.no_grad():
                ...
                if cur_val_loss < global_loss:
                    ...
                    torch.save(state, './save/temporal/lstm')
                    print('\n------------ Save best model ------------\n')
...
with torch.no_grad():
    for v_i in tqdm(range(0, len(test_ehr_dataset), config.batch_size)):
        ...
        # Get batch outputs
        test_loss, predictions, labels = model(batch_ehr, ehr_labels=batch_ehr, ehr_masks=batch_mask)
...
pickle.dump(intermediate, open("./results/temporal_completion_stats/LSTM_intermediate_results.pkl", "wb"))
...
pickle.dump(metrics_dict, open("./results/temporal_completion_stats/LSTM_Metrics.pkl", "wb"))
