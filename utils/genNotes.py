...
noteFile = mimic_dir + "NOTEEVENTS.csv"
...
noteDf = pd.read_csv(noteFile, dtype=str)
noteDf = noteDf[noteDf.CATEGORY == 'Discharge summary']
noteDf = noteDf[noteDf.DESCRIPTION == 'Report']
noteDf = noteDf.drop_duplicates('HADM_ID', keep='last')
noteDf = noteDf.set_index("HADM_ID")
noteDf = noteDf[['TEXT']]
noteDf = noteDf[noteDf['TEXT'].notnull()]
noteDf['TEXT'] = noteDf['TEXT'].str.replace('\r',' ')
noteDf['TEXT'] = noteDf['TEXT'].apply(str.strip)
noteDf['TEXT'] = noteDf['TEXT'].str.lower()
noteDf['TEXT'] = noteDf['TEXT'].apply(lambda x: re.sub('\\[(.*?)\\]','[DEIDENTIFIED]',x))

noteAdmits = set(noteDf.index.unique())
admissionDf = admissionDf[admissionDf.HADM_ID.isin(noteAdmits)]
...
for row in tqdm(admissionDf.itertuples(), total=admissionDf.shape[0]):          
    ...
    note = noteDf.loc[[hadm_id]]["TEXT"].values[0]
    note = tokenizer.encode(note)
        
    # Building the hospital admission data point
    if subject_id not in data:
      data[subject_id] = {'visits': [(diagnoses, procedures, medications, note)]}
    else:
      data[subject_id]['visits'].append((diagnoses, procedures, medications, note))
...
for p in data:
    ...
    for v in data[p]['visits']:
        ...
        new_visits.append((list(set(new_visit)), v[3]))
...
print(f"MAX NOTE LEN: {max([len(v[1]) for p in data for v in p['visits']])}")
print(f"AVG NOTE LEN: {np.mean([len(v[1]) for p in data for v in p['visits']])}")
print(f"90TH PERCENTILE NOTE LEN: {np.percentile([len(v[1]) for p in data for v in p['visits']], 90)}")
...
pickle.dump(code_to_index, open("../data_note/codeToIndex.pkl", "wb"))
pickle.dump(index_to_code, open("../data_note/indexToCode.pkl", "wb"))
pickle.dump(train_dataset, open("../data_note/trainDataset.pkl", "wb"))
pickle.dump(val_dataset, open("../data_note/valDataset.pkl", "wb"))
pickle.dump(test_dataset, open("../data_note/testDataset.pkl", "wb"))