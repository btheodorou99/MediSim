...
def finish_sequence(model, length, context, rest, sample=True):
  prev = context
  with torch.no_grad():
    for i in range(length-context.size(1)):
      toAdd = rest[:,i,:].unsqueeze(1)
      toAdd[:,:,config.diagnosis_vocab_size:config.diagnosis_vocab_size+config.procedure_vocab_size+config.medication_vocab_size] = 0
      prev = model.addModalities(torch.cat((prev,toAdd), dim=1), sample)
...
with torch.no_grad():
  for i in tqdm(range(0, len(dataset), config.sample_batch_size)):
    ...
    predictions = finish_sequence(model, config.n_ctx, batch_ehr[:,:1,:], batch_ehr[:,1:,:], sample=True)
    batch_converted_ehr = convert_ehr(predictions)
    ehr_dataset += batch_converted_ehr
    
pickle.dump(ehr_dataset, open('./results/datasets/modality_added_ss.pkl', 'wb'))
