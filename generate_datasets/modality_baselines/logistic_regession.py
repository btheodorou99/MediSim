...
def finish_sequence(model, length, record, sample=True):
  with torch.no_grad():
    for i in range(1, length):
      record[:,i,config.diagnosis_vocab_size:config.diagnosis_vocab_size+config.procedure_vocab_size+config.medication_vocab_size] = torch.bernoulli(torch.tensor(np.array(model.predict_proba(record[:,i,:config.diagnosis_vocab_size].numpy())).transpose((1,0,2)))[:,:,1])
...
pickle.dump(ehr_dataset, open('./results/datasets_modality_baselines/modality_added_logistic_regression.pkl', 'wb'))
