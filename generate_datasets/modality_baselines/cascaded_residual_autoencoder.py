...
def finish_sequence(model, length, record, sample=True):
  with torch.no_grad():
    for i in range(1, length):
      record[:,i,config.diagnosis_vocab_size:config.diagnosis_vocab_size+config.procedure_vocab_size+config.medication_vocab_size] = torch.bernoulli(model(record[:,i,:config.diagnosis_vocab_size])[1])
...
pickle.dump(ehr_dataset, open('./results/datasets_modality_baselines/modality_added_cascaded_residual_autoencoder.pkl', 'wb'))
