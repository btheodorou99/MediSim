  ...
  for e in tqdm(range(config.epoch)):
    ...
    with torch.no_grad():
      ...
      if cur_val_loss < global_loss:
        ...
        torch.save(state, './save/medisim_model_image_ss')
        print('\n------------ Save best model ------------\n')
      else:
        curr_patience += 1

  model.load_state_dict(torch.load('./save/medisim_model_image_ss')['model'])
...
def train_quality_model(quality_dataset):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=QUALITY_LR)
    ...
    for e in range(QUALITY_EPOCHS):
      ...
      with torch.no_grad():
        ...
        if cur_val_loss < quality_loss:
          ...
          torch.save(state, f'./save/quality_model_image')
          print('\n------------ Save best model ------------\n')

    model.load_state_dict(torch.load('./save/quality_model_image')['model'])
    return model
...
if os.path.exists("./save/medisim_model_image_base"):
